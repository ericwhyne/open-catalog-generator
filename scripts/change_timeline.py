				...
				for (data in office_data) {	//for each program within an office
					...
					if(office_data[data].projects){ //The program has projects that were added or updated 
						...
						if(change_dates[i] == office_data[data]["Date"].getTime()){
							...
							html[i] += "<p style='text-align:left; width:100%;'><span class='" + type_class + "'>" + office_data[data]["Date Type"] + "</span> <a href=" + url_redirect + "><span class='slide_project_span' style='color: #" + office_data[data].office["DARPA Office Color"] + ";'>" + office_data[data].program + "</span></a></p>";
						}
					}
